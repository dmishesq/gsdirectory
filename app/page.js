...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <section className="py-16 text-center"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><h2 className="text-2xl md:text-3xl font-display text-brand-950 mb-12">Why List on Global Site Directory?</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-8"><div><h3 className="font-display text-lg text-brand-900 mb-2">Free Dofollow Backlink</h3><p className="text-sm text-gray-600">Every listing includes a dofollow link to your website. Build domain authority for free.</p></div><div><h3 className="font-display text-lg text-brand-900 mb-2">Get Discovered</h3><p className="text-sm text-gray-600">Your listing is indexed by Google. Customers searching for your services find you here.</p></div><div><h3 className="font-display text-lg text-brand-900 mb-2">Verified & Trusted</h3><p className="text-sm text-gray-600">Claim your listing for a verified badge. Build trust with potential customers.</p></div></div><div className="mt-12"><a href="/submit/" className="bg-brand-900 hover:bg-brand-800 text-white font-semibold px-8 py-3.5 rounded-lg">Add Your Business Now</a></div></div></section>
      <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify({'@context':'https://schema.org','@type':'WebSite',name:'Global Site Directory',url:'https://gsdirectory.org',description:'Free business directory with dofollow backlinks.'})}} />
    </>
  )
}
